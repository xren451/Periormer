...
For Informer, we set the number of attention heads $h$ to 16 and the stride in the max pooling to 2. 
...
For Reformer, the number of layers is set to 3. The embedding size is set to 1024 in \textsf{Reformer} and 512 for the other approaches. $R^{2}$ is set to 80\% for the truncation of the high component  in Periormer.
